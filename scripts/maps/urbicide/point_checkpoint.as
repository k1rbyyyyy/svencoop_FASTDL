...
enum PointCheckpointFlags
{
	...
	SF_CHECKPOINT_STARTINACTIVE	= 1 << 1,	//This checkpoint starts disabled, trigger to enable
	SF_DONT_EQUIP_SPAWNED		= 1 << 2,	//Disable spawned players from triggering game_player_equips
}

bool blPointCheckPointRegistered = RegisterPointCheckPointEntity();

bool RegisterPointCheckPointEntity()
{
...
class point_checkpoint : ScriptBaseAnimating
{
	...
	// Sound to play when the checkpoint is activated
	private string m_sActivationMusic			= "../media/valve.mp3";
	...
	// Keep track of whether the entity is initialised or not
	...
	bool KeyValue( const string& in szKey, const string& in szValue )
	{
		...
		else if( szKey == "m_sActivationMusic" )
		{
	...
	// If you're going use this in your script make sure you don't try to access invalid animations.
	...
	void Spawn()
	{
		...
		if( self.pev.SpawnFlagBitSet( SF_CHECKPOINT_STARTINACTIVE ) )
		{
	...
	void Touch( CBaseEntity@ pOther )
	{
		...
		bool blCanActivate = false;

		for( int iPlayer = 1; iPlayer <= g_Engine.maxClients; iPlayer++ )
		{
			CBasePlayer@ pPlayer = g_PlayerFuncs.FindPlayerByIndex( iPlayer );

			if( pPlayer is null || pPlayer is pOther || !pPlayer.IsConnected() )
				continue;
		...
		// Set activated
		self.pev.frags = 1.0f;
		g_PlayerFuncs.ClientPrintAll( HUD_PRINTNOTIFY, string( pOther.pev.netname ) + " activated a checkpoint.\n" );

		if( !m_sActivationMusic.IsEmpty() )
			g_SoundSystem.EmitSound( self.edict(), CHAN_STATIC, m_sActivationMusic, 1.0f, ATTN_NONE );
	...
	void RespawnThink()
	{
		...
		for( ; m_iNextPlayerToRevive <= g_Engine.maxClients; ++m_iNextPlayerToRevive )
		{
			...
			if( pPlayer !is null && !pPlayer.IsAlive() && pPlayer.m_fDeadTime < m_flRespawnStartTime )
			{
				...
				if( !self.pev.SpawnFlagBitSet( SF_DONT_EQUIP_SPAWNED ) )
				{
					...
					while ( ( @pEquipEntity = g_EntityFuncs.FindEntityByClassname( pEquipEntity, "game_player_equip" ) ) !is null  )
						pEquipEntity.Use( pPlayer, self, USE_TOGGLE, 0.0f );
				}
				//Fire target entity on player respawn (if specified)
				if ( !string( self.pev.target ).IsEmpty() )
					g_EntityFuncs.FireTargets( self.pev.target, pPlayer, self, USE_TOGGLE );
				//Trigger "message" for every player spawned
