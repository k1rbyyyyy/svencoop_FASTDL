...
* Modified by: Josh "JPolito" Polito, Tomas "GeckoN" Slavotinek
*/

namespace VehicleCustom
{
...
const int VEHICLE_STARTPITCH = 100;
...
class func_vehicle_custom : ScriptBaseEntity
{
	...
	void Precache()
	{
		...
		g_SoundSystem.PrecacheSound("th_escape/diesel_idle_loop.flac"); self.pev.noise = "th_escape/diesel_idle_loop.flac";
	...
	void SetDriver( CBasePlayer@ pDriver )
	{
		...
		if( pDriver !is null )
			g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_ITEM, "th_escape/diesel_rev.flac", 0.8, ATTN_NORM, 0, PITCH_NORM );
	}
...
{
	SF_VEHICLE_RC = 1 << 0, //This func_vehiclecontrols_custom is a remote control, not driver control
}

class func_vehiclecontrols_custom : ScriptBaseEntity
{
	int ObjectCaps()
	{
...
void RegisterHooks( bool fAddDebugCode = false )
{
...
void RemoveHooks()
{
	g_Hooks.RemoveHook( Hooks::Player::PlayerUse, @VehiclePlayerUse );
	g_Hooks.RemoveHook( Hooks::Player::PlayerPreThink, @VehiclePlayerPreThink );
	g_Hooks.RemoveHook( Hooks::Player::ClientPutInServer, @VehicleClientPutInServer );
	
	g_Hooks.RemoveHook( Hooks::Player::ClientSay, @VehicleClientSay );
}

void Register( bool fRegisterHooks = true, bool fAddDebugCode = false )
{
	if ( fRegisterHooks )
	{
		RegisterHooks( fAddDebugCode );
	}
	
	g_CustomEntityFuncs.RegisterCustomEntity( "VehicleCustom::func_vehicle_custom", "func_vehicle_custom" );
	g_CustomEntityFuncs.RegisterCustomEntity( "VehicleCustom::func_vehiclecontrols_custom", "func_vehiclecontrols_custom" );
}
