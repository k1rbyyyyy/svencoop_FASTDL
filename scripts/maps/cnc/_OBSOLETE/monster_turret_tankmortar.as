...
//			monster_turret, which fires mortar shells.
//
//==============================================================//
/*
...
*/
class monster_turret_tankmortar : ScriptBaseMonsterEntity
{
		CSprite@ m_pEyeGlow;
		int	m_eyeBrightness;
	...
	{ 
		...
		g_EntityFuncs.DispatchKeyValue( self.edict(), "displayname", "Turret Tank Mortar" );
	...
	void Precache( void )
	{
		...
		g_SoundSystem.PrecacheSound( "ambience/biggun3.wav" );
		...
		g_sModelIndexShellSmoke = g_Game.PrecacheModel( SHELL_SMOKE );//Shell smoke
	...
	{		
		if ( m_fireLast != 0 ) 
		{
			...
			if (rocketcount > 0) 
			{
				@rocketquided = g_EntityFuncs.ShootMortar( self.pev, vecSrc, g_Engine.v_forward * 3000);
				
				//Can't set a high speed without making the console whinning? No Problemo, off the gravity goes
				rocketquided.pev.movetype = MOVETYPE_BOUNCEMISSILE;
				
				g_SoundSystem.EmitSoundDyn( self.edict(), CHAN_WEAPON, "ambience/biggun3.wav", 1, 0.6, 0, 120);
				
				self.pev.effects = pev.effects | EF_MUZZLEFLASH;
				
				NetworkMessage gunsmoke( MSG_PVS, NetworkMessages::SVC_TEMPENTITY );
				gunsmoke.WriteByte( TE_BEAMFOLLOW );
				gunsmoke.WriteShort( rocketquided.entindex() );
				gunsmoke.WriteShort( g_sModelIndexShellSmoke );
				gunsmoke.WriteByte( 40 ); // life
				gunsmoke.WriteByte( 5 ); // width
				gunsmoke.WriteByte( 104 ); // r, g, b
				gunsmoke.WriteByte( 104 ); // r, g, b
				gunsmoke.WriteByte( 125 ); // r, g, b
				gunsmoke.WriteByte( 125 ); // brightness
				gunsmoke.End();
	...
	int TakeDamage(entvars_t@ pevInflictor, entvars_t@ pevAttacker, float flDamage, int bitsDamageType)
	{
		...
		if ( ( ptest !is null ) && ( bitsDamageType == DMG_CLUB ) && CanRepairTurret( ptest ) && ( self.IRelationship( ptest ) <= R_NO ) )
		{
			//Heal
			bIsHealing = true; 
			self.TakeHealth( 25, DMG_CLUB, 1000);
		}
...
string GetTurretTankMortarName()
{
	return "monster_turret_tankmortar";
}

void RegisterTurretTankMortar()
{
	g_CustomEntityFuncs.RegisterCustomEntity( "monster_turret_tankmortar", GetTurretTankMortarName() );
}