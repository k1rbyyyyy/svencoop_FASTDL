		...
		else
		{
			...
			if( pEntity !is null )
			{
				...
				if( pEntity.IsMachine() && pEntity.IsPlayerAlly() )
				{
					...
					{
						// KernCore: Calculate amount to heal and heal based on the damage value
						pEntity.pev.health += (pEntity.pev.health + flDamage < pEntity.pev.max_health) ? flDamage : pEntity.pev.max_health - pEntity.pev.health;
			...
			if( fHitWorld )
			{
				...
				switch( Math.RandomLong( 0, 1 ) )
				{
				case 0: g_SoundSystem.EmitSoundDyn( pPlayer.edict(), CHAN_WEAPON, "weapons/pwrench_hit1.wav", (pEntity.IsMachine() && pEntity.IsPlayerAlly()) ? 1.0f : fvolbar, 
					ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); break;
				case 1: g_SoundSystem.EmitSoundDyn( pPlayer.edict(), CHAN_WEAPON, "weapons/pwrench_hit2.wav", (pEntity.IsMachine() && pEntity.IsPlayerAlly()) ? 1.0f : fvolbar, 
					ATTN_NORM, 0, 98 + Math.RandomLong( 0, 3 ) ); break;
				}
			}
