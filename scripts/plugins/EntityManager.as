 ...
 *
 * This plug-in was intended for use by mappers to aid map debugging. Not for
 * public games!
 *
 * Before enabling this plug-in on your server please be aware this plug-in has
 * NO ACCESS RESTRICTION on any of its features. This means that anyone that
 * discovers this plug-in is running (and they can with just one command) can
 * use this VERY abusively: It has a LOT of power in how your server's maps will
 * run.
 *
 * To restrict whom can use this plug-ins features you must declare so with the
 * "adminlevel" option in your plug-ins configuration. E.g. to limit use to
 * server administrators only, include
 *     "adminlevel" "ADMIN_YES"
 * below
 *     "script" "EntityManager"
 * or to only allow the server owner use of this, specify value "ADMIN_OWNER".
 *
 */
